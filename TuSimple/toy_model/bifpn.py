...
class BiFPNLastLayer(nn.Module):
    def __init__(self, num_features=5, channels=64):
        ...
        self.up_conv2 = ConvBNReLU(channels, channels, 3, padding=1, with_relu=False)
        self.up_conv3 = ConvBNReLU(channels, channels, 3, padding=1, with_relu=False)
        self.up_conv4 = ConvBNReLU(channels, channels, 3, padding=1, with_relu=False)
        self.up_conv5 = ConvBNReLU(channels, channels, 3, padding=1, with_relu=False)
    def forward(self, features):
        p2_in, p3_in, p4_in, p5_in = features
        up_4 = self.up5(p5_in)
        up_4 = self.up_conv4(self.up_act4(p4_in + up_4))
        up_3 = self.up4(up_4)
        up_3 = self.up_conv3(self.up_act3(p3_in + up_3))
        up_2 = self.up2(up_3)
        up_2 = self.up_conv2(self.up_act2(p2_in + up_2))
...
class BiFPNLayer(nn.Module):
    ...
    def __init__(self, num_features=5, channels=64):
        ...
        self.down_conv3 = ConvBNReLU(channels, channels, 3, padding=1, with_relu=False)
        self.down_conv4 = ConvBNReLU(channels, channels, 3, padding=1, with_relu=False)
        self.down_conv5 = ConvBNReLU(channels, channels, 3, padding=1, with_relu=False)
    ...
    def forward(self, features):
        ...
        down_3 = self.down_conv3(self.down_act3(down_3 + up_3 + p3_in))
        down_4 = self.down3(down_3)
        down_4 = self.down_conv4(self.down_act4(down_4 + up_4 + p4_in))
        down_5 = self.down4(down_4)
        down_5 = self.down_conv5(self.down_act5(down_5 + p5_in + p5_in))
...
class BiFPN(nn.Module):
    ...
    def __init__(self, encoder_channels, pyramid_channels=32, num_layers=2, output_index=[0]):
        super().__init__()
    ...
    def forward(self, features):
        ...
        return [features[i] for i in self.output_index] 
